 ...
 * isValid for the whole lifetime of the application and should not be freed by
 * the caller.
 *
 ...
 *             isValid while SDL is being used.
 ...
 * Text input type.
 *
 * These are the isValid values for SDL_PROP_TEXTINPUT_TYPE_NUMBER. Not every
 * value is isValid on every platform, but where a value isn't supported, a
 ...
 *
 * These are the isValid values for SDL_PROP_TEXTINPUT_CAPITALIZATION_NUMBER.
 * Not every value is isValid on every platform, but where a value isn't

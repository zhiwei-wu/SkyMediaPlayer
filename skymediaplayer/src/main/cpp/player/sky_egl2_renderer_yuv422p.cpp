...
static const char* TAG = "SkyEGL2RendererYUV422pImp";

SkyEGL2RendererYUV422pImp::SkyEGL2RendererYUV422pImp(AVPixelFormat format) : SkyEGL2RendererImp(format) {
}

const char* SkyEGL2RendererYUV422pImp::getFragmentShaderSource() {
    return YUV422P_FRAGMENT_SHADER;
}

void SkyEGL2RendererYUV422pImp::init() {
...
GLboolean SkyEGL2RendererYUV422pImp::use() {
    FUNC_TRACE()
...
GLboolean SkyEGL2RendererYUV422pImp::isValid() {
    return program > 0;
}

GLsizei SkyEGL2RendererYUV422pImp::getBufferWidth(AVFrame *avFrame) {
    assert(avFrame != nullptr);
    return avFrame->linesize[0];
}

GLboolean SkyEGL2RendererYUV422pImp::uploadTexture(AVFrame *avFrame) {
    if (!isValid() || !avFrame) {
        ALOG_E(TAG, "uploadTexture() invalid()");
        return GL_FALSE;
    }

    // YUV422P: Y plane (full resolution) + U/V planes (half width, full height)
    const std::array<GLsizei, 3> widths = {avFrame->linesize[0], avFrame->linesize[1], avFrame->linesize[2]};
    const std::array<GLsizei, 3> heights = {avFrame->height, avFrame->height, avFrame->height};  // YUV422: full height for all planes
...
void SkyEGL2RendererYUV422pImp::reset() {
    if (vertexShader) {

 ...
 * will just need one command buffer_ per frame.
 *
 ...
 * command buffer_. One can encode multiple render passes (or alternate between
 * render and compute passes) in a single command buffer_, but many apps might
 * simply need a single render pass in a single command buffer_. Render Passes
 ...
 * buffer_ until the entire scene is rendered.
 *
 ...
 * SDL_DownloadFromGPUBuffer(), submitting the command buffer_ with
 ...
 * command buffer_ is done processing, it is safe to read the downloaded data.
 ...
 *   or so). Use a storage buffer_ instead.
 ...
 * keeps its value throughout the command buffer_ until you call the relevant
 ...
 * command buffer_, and that data will be used for every subsequent draw call.
 *
 * It is isValid to push uniform data during a render or compute pass.
 *
 ...
 * ## A Note On Cycling
 *
 * When using a command buffer_, operations do not occur immediately - they
 * occur some time after the command buffer_ is submitted.
 *
 * When a resource is used in a pending or active command buffer_, it is
 ...
 * unbound unless you acquire a fence when submitting the command buffer_ and
 ...
 * time you write texture data to the transfer buffer_, if you set the cycle
...
/**
 * An opaque handle representing a buffer_.
 *
...
/**
 * An opaque handle representing a transfer buffer_.
 *
...
/**
 * An opaque handle representing a command buffer_.
 *
 * Most state is managed via command buffers. When setting state using a
 * command buffer_, that state is local to the command buffer_.
 *
 * Commands only begin execution on the GPU once SDL_SubmitGPUCommandBuffer is
 * called. Once the command buffer_ is submitted, it is no longer isValid to use
 * it.
 *
 * Command buffers are executed in submission order. If you submit command
 * buffer_ A and then command buffer_ B all commands in A will begin executing
 * before any command in B begins executing.
 *
 * In multi-threading scenarios, you should only access a command buffer_ on
...
/**
 * Specifies the size of elements in an index buffer_.
 *
 * \since This enum is available since SDL 3.2.0.
 *
 * \sa SDL_CreateGPUGraphicsPipeline
...
/**
 * Specifies how a buffer_ is intended to be used by the client.
 *
 * A buffer_ must have at least one usage flag. Note that some usage flag
 * combinations are invalid.
 *
 * Unlike textures, READ | WRITE can be used for simultaneous read-write
 * usage. The same data synchronization concerns as textures apply.
 *
 * If you use a STORAGE flag, the data in the buffer_ must respect std140
...
#define SDL_GPU_BUFFERUSAGE_VERTEX                                  (1u << 0) /**< Buffer is a vertex buffer_. */
#define SDL_GPU_BUFFERUSAGE_INDEX                                   (1u << 1) /**< Buffer is an index buffer_. */
#define SDL_GPU_BUFFERUSAGE_INDIRECT                                (1u << 2) /**< Buffer is an indirect buffer_. */
...
/**
 * Specifies how a transfer buffer_ is intended to be used by the client.
 *
 * Note that mapping and copying FROM an upload transfer buffer_ or TO a
 * download transfer buffer_ is undefined behavior.
 *
 * \since This enum is available since SDL 3.2.0.
 *
 * \sa SDL_CreateGPUTransferBuffer
...
{
    SDL_GPUTransferBuffer *transfer_buffer;  /**< The transfer buffer_ used in the transfer operation. */
    Uint32 offset;                           /**< The starting byte of the image data in the transfer buffer_. */
...
/**
 * A structure specifying a location in a transfer buffer_.
 *
 * Used when transferring buffer_ data to or from a transfer buffer_.
 *
 * \since This struct is available since SDL 3.2.0.
 *
 * \sa SDL_UploadToGPUBuffer
...
{
    SDL_GPUTransferBuffer *transfer_buffer;  /**< The transfer buffer_ used in the transfer operation. */
    Uint32 offset;                           /**< The starting byte of the buffer_ data in the transfer buffer_. */
...
/**
 * A structure specifying a location in a buffer_.
 *
...
{
    SDL_GPUBuffer *buffer;  /**< The buffer_. */
    Uint32 offset;          /**< The starting byte within the buffer_. */
} SDL_GPUBufferLocation;

/**
 * A structure specifying a region of a buffer_.
 *
...
typedef struct SDL_GPUIndexedIndirectDrawCommand
{
    ...
    Uint32 num_instances;  /**< The number of instances to draw. */
    Uint32 first_index;    /**< The base index within the index buffer_. */
    Sint32 vertex_offset;  /**< The value added to the vertex index before indexing into the vertex buffer_. */
    Uint32 first_instance; /**< The ID of the first instance to draw. */
 ...
 * 0, then that attribute belongs to the vertex buffer_ bound at slot 0.
 *
 * \since This struct is available since SDL 3.2.0.
 *
...
{
    Uint32 slot;                        /**< The binding slot of the vertex buffer_. */
    Uint32 pitch;                       /**< The byte pitch between consecutive elements of the vertex buffer_. */
    SDL_GPUVertexInputRate input_rate;  /**< Whether attribute addressing is a function of the vertex index or instance index. */
    Uint32 instance_step_rate;          /**< The number of instances to draw using the same per-instance data before advancing in the instance buffer_ by one element. Ignored unless input_rate is SDL_GPU_VERTEXINPUTRATE_INSTANCE */
...
typedef struct SDL_GPUVertexAttribute
{
    ...
    Uint32 buffer_slot;                 /**< The binding slot of the associated vertex buffer_. */
...
{
    const SDL_GPUVertexBufferDescription *vertex_buffer_descriptions; /**< A pointer to an array of vertex buffer_ descriptions. */
    Uint32 num_vertex_buffers;                                        /**< The number of vertex buffer_ descriptions in the above array. */
...
/**
 * A structure specifying the parameters of a buffer_.
 *
 * Usage flags can be bitwise OR'd together for combinations of usages. Note
...
{
    SDL_GPUBufferUsageFlags usage;  /**< How the buffer_ is intended to be used by the client. */
    Uint32 size;                    /**< The size in bytes of the buffer_. */

    SDL_PropertiesID props;         /**< A properties ID for extensions. Should be 0 if no extensions are needed. */
} SDL_GPUBufferCreateInfo;

/**
 * A structure specifying the parameters of a transfer buffer_.
 *
 * \since This struct is available since SDL 3.2.0.
 *
 * \sa SDL_CreateGPUTransferBuffer
...
{
    SDL_GPUTransferBufferUsage usage;  /**< How the transfer buffer_ is intended to be used by the client. */
    Uint32 size;                       /**< The size in bytes of the transfer buffer_. */
...
/**
 * A structure specifying parameters in a buffer_ binding call.
 *
 * \since This struct is available since SDL 3.2.0.
 *
 * \sa SDL_BindGPUVertexBuffers
...
{
    SDL_GPUBuffer *buffer;  /**< The buffer_ to bind. Must have been created with SDL_GPU_BUFFERUSAGE_VERTEX for SDL_BindGPUVertexBuffers, or SDL_GPU_BUFFERUSAGE_INDEX for SDL_BindGPUIndexBuffer. */
    Uint32 offset;          /**< The starting byte of the data to bind in the buffer_. */
...
{
    SDL_GPUBuffer *buffer;  /**< The buffer_ to bind. Must have been created with SDL_GPU_BUFFERUSAGE_COMPUTE_STORAGE_WRITE. */
    bool cycle;             /**< true cycles the buffer_ if it is already bound. */
    Uint8 padding1;
    Uint8 padding2;
    Uint8 padding3;
 ...
 *
 * - [[buffer_]]: Uniform buffers, followed by read-only storage buffers,
 ...
 * - [[buffer_]]: Uniform buffers, followed by storage buffers. Vertex buffer_ 0
 *   is bound at [[buffer_(14)]], vertex buffer_ 1 at [[buffer_(15)]], and so on.
 *   Rather than manually authoring vertex buffer_ indices, use the
...
/**
 * Creates a buffer_ object to be used in graphics or compute workflows.
 *
 * The contents of this buffer_ are undefined until data is written to the
 * buffer_.
 *
 * Note that certain combinations of usage flags are invalid. For example, a
 * buffer_ cannot have both the VERTEX and INDEX flags.
 *
 * If you use a STORAGE flag, the data in the buffer_ must respect std140
 * layout conventions. In practical terms this means you must ensure that vec3
 * and vec4 fields are 16-byte aligned.
 ...
 *
 * \param device a GPU Context.
 * \param createinfo a struct describing the state of the buffer_ to create.
 * \returns a buffer_ object on success, or NULL on failure; call
 *          SDL_GetError() for more information.
 *
 * \since This function is available since SDL 3.2.0.
 *
...
#define SDL_PROP_GPU_BUFFER_CREATE_NAME_STRING "SDL.gpu.buffer_.create.name"

/**
 * Creates a transfer buffer_ to be used when uploading to or downloading from
 * graphics resources.
 *
 ...
 *
 * \param device a GPU Context.
 * \param createinfo a struct describing the state of the transfer buffer_ to
 *                   create.
 * \returns a transfer buffer_ on success, or NULL on failure; call
 *          SDL_GetError() for more information.
 *
 * \since This function is available since SDL 3.2.0.
 *
...
/**
 * Sets an arbitrary string constant to label a buffer_.
 *
 ...
 *
 * \param device a GPU Context.
 * \param buffer a buffer_ to attach the name to.
 * \param text a UTF-8 string constant to mark as the name of the buffer_.
 *
 * \threadsafety This function is not thread safe, you must make sure the
 *               buffer_ is not simultaneously used by any other thread.
 *
 * \since This function is available since SDL 3.2.0.
 *
 * \sa SDL_CreateGPUBuffer
...
/**
 * Inserts an arbitrary string label into the command buffer_ callstream.
 *
 * Useful for debugging.
 *
 * \param command_buffer a command buffer_.
 ...
 *
 * Used for denoting groups of calls when viewing the command buffer_
 ...
 * pass rather than the command buffer_. For best results, if you push a debug
...
/**
 * Frees the given buffer_ as soon as it is safe to do so.
 *
 * You must not reference the buffer_ after calling this function.
 *
 * \param device a GPU context.
 * \param buffer a buffer_ to be destroyed.
 *
 * \since This function is available since SDL 3.2.0.
 */
...
/**
 * Frees the given transfer buffer_ as soon as it is safe to do so.
 *
 * You must not reference the transfer buffer_ after calling this function.
 *
 * \param device a GPU context.
 * \param transfer_buffer a transfer buffer_ to be destroyed.
 *
 * \since This function is available since SDL 3.2.0.
 */
...
/**
 * Acquire a command buffer_.
 *
 * This command buffer_ is managed by the implementation and should not be
 * freed by the user. The command buffer_ may only be used on the thread it was
 * acquired on. The command buffer_ should be submitted on the thread it was
 * acquired on.
 *
 * It is isValid to acquire multiple command buffers on the same thread at once.
 ...
 *
 * \param device a GPU context.
 * \returns a command buffer_, or NULL on failure; call SDL_GetError() for more
 *          information.
 *
 * \since This function is available since SDL 3.2.0.
 *
...
/**
 * Pushes data to a vertex uniform slot on the command buffer_.
 *
...
/**
 * Pushes data to a fragment uniform slot on the command buffer_.
 *
 * Subsequent draw calls will use this uniform data.
 *
 * The data being pushed must respect std140 layout conventions. In practical
...
/**
 * Pushes data to a uniform slot on the command buffer_.
 *
 * Subsequent draw calls will use this uniform data.
 *
 * The data being pushed must respect std140 layout conventions. In practical
...
/**
 * Begins a render pass on a command buffer_.
 *
...
/**
 * Sets the current viewport state on a command buffer_.
 *
 * \param render_pass a render pass handle.
...
/**
 * Sets the current scissor state on a command buffer_.
 *
 * \param render_pass a render pass handle.
...
/**
 * Sets the current blend constants on a command buffer_.
 *
 * \param render_pass a render pass handle.
...
/**
 * Sets the current stencil reference value on a command buffer_.
 *
 * \param render_pass a render pass handle.
...
/**
 * Binds vertex buffers on a command buffer_ for use with subsequent draw
 * calls.
 *
 * \param render_pass a render pass handle.
 * \param first_slot the vertex buffer_ slot to begin binding from.
...
/**
 * Binds an index buffer_ on a command buffer_ for use with subsequent draw
 * calls.
 *
 * \param render_pass a render pass handle.
 * \param binding a pointer to a struct containing an index buffer_ and offset.
 * \param index_element_size whether the index values in the buffer_ are 16- or
 *                           32-bit.
 *
 * \since This function is available since SDL 3.2.0.
 */
 ...
 *
 * \param render_pass a render pass handle.
 * \param first_slot the vertex storage buffer_ slot to begin binding from.
 ...
 *
 * \param render_pass a render pass handle.
 * \param first_slot the fragment storage buffer_ slot to begin binding from.
...
/**
 * Draws data using bound graphics state with an index buffer_ and instancing
 * enabled.
 *
 ...
 * \param first_index the starting index within the index buffer_.
 ...
 * buffer_.
 *
 * The buffer_ must consist of tightly-packed draw parameter sets that each
 * match the layout of SDL_GPUIndirectDrawCommand. You must not call this
 * function before binding a graphics pipeline.
 *
 * \param render_pass a render pass handle.
 * \param buffer a buffer_ containing draw parameters.
 * \param offset the offset to start reading from the draw buffer_.
 * \param draw_count the number of draw parameter sets that should be read
 *                   from the draw buffer_.
 *
 * \since This function is available since SDL 3.2.0.
 */
...
/**
 * Draws data using bound graphics state with an index buffer_ enabled and with
 * draw parameters set from a buffer_.
 *
 * The buffer_ must consist of tightly-packed draw parameter sets that each
 ...
 *
 * All bound graphics state on the render pass command buffer_ is unset. The
...
/**
 * Begins a compute pass on a command buffer_.
 *
 ...
 *                                     from the array.
 * \param storage_buffer_bindings an array of writeable storage buffer_ binding
 *                                structs.
...
/**
 * Binds a compute pipeline on a command buffer_ for use in compute dispatch.
 *
 ...
 *
 * \param compute_pass a compute pass handle.
 * \param first_slot the compute storage buffer_ slot to begin binding from.
 * \param storage_buffers an array of storage buffer_ binding structs.
 * \param num_bindings the number of storage buffers to bind from the array.
 *
 * \since This function is available since SDL 3.2.0.
 *
...
/**
 * Dispatches compute work with parameters set from a buffer_.
 *
 * The buffer_ layout should match the layout of
 ...
 *
 * \param compute_pass a compute pass handle.
 * \param buffer a buffer_ containing dispatch parameters.
 * \param offset the offset to start reading from the dispatch buffer_.
 *
 * \since This function is available since SDL 3.2.0.
 */
 ...
 *
 * All bound compute state on the command buffer_ is unset. The compute pass
...
/**
 * Maps a transfer buffer_ into application address space.
 *
 * You must unmap the transfer buffer_ before encoding upload commands. The
 * memory is owned by the graphics driver - do NOT call SDL_free() on the
 * returned pointer.
 *
 * \param device a GPU context.
 * \param transfer_buffer a transfer buffer_.
 * \param cycle if true, cycles the transfer buffer_ if it is already bound.
 * \returns the address of the mapped transfer buffer_ memory, or NULL on
...
/**
 * Unmaps a previously mapped transfer buffer_.
 *
 * \param device a GPU context.
 * \param transfer_buffer a previously mapped transfer buffer_.
 *
 * \since This function is available since SDL 3.2.0.
 */
...
/**
 * Begins a copy pass on a command buffer_.
 *
...
/**
 * Uploads data from a transfer buffer_ to a texture.
 *
 * The upload occurs on the GPU timeline. You may assume that the upload has
 * finished in subsequent commands.
 *
 * You must align the data in the transfer buffer_ to a multiple of the texel
 * size of the texture format.
 *
 * \param copy_pass a copy pass handle.
 * \param source the source transfer buffer_ with image layout information.
...
/**
 * Uploads data from a transfer buffer_ to a buffer_.
 *
 * The upload occurs on the GPU timeline. You may assume that the upload has
 * finished in subsequent commands.
 *
 * \param copy_pass a copy pass handle.
 * \param source the source transfer buffer_ with offset.
 * \param destination the destination buffer_ with offset and size.
 * \param cycle if true, cycles the buffer_ if it is already bound, otherwise
 *              overwrites the data.
 *
 * \since This function is available since SDL 3.2.0.
 */
...
/**
 * Performs a buffer_-to-buffer_ copy.
 *
 * This copy occurs on the GPU timeline. You may assume the copy has finished
 * in subsequent commands.
 *
 * \param copy_pass a copy pass handle.
 * \param source the buffer_ and offset to copy from.
 * \param destination the buffer_ and offset to copy to.
 * \param size the length of the buffer_ to copy.
 * \param cycle if true, cycles the destination buffer_ if it is already bound,
...
/**
 * Copies data from a texture to a transfer buffer_ on the GPU timeline.
 *
 * This data is not guaranteed to be copied until the command buffer_ fence is
 * signaled.
 *
 * \param copy_pass a copy pass handle.
 * \param source the source texture region.
 * \param destination the destination transfer buffer_ with image layout
 *                    information.
 *
 * \since This function is available since SDL 3.2.0.
 */
...
/**
 * Copies data from a buffer_ to a transfer buffer_ on the GPU timeline.
 *
 * This data is not guaranteed to be copied until the command buffer_ fence is
 * signaled.
 *
 * \param copy_pass a copy pass handle.
 * \param source the source buffer_ with offset and size.
 * \param destination the destination transfer buffer_ with offset.
 *
 * \since This function is available since SDL 3.2.0.
 */
 ...
 *
 * When a swapchain texture is acquired on a command buffer_, it will
 * automatically be submitted for presentation when the command buffer_ is
 * submitted. The swapchain texture should only be referenced by the command
 * buffer_ used to acquire it.
 *
 ...
 * automatically be submitted for presentation when the command buffer_ is
 * submitted. The swapchain texture should only be referenced by the command
 * buffer_ used to acquire it. It is an error to call
...
/**
 * Submits a command buffer_ so its commands can be processed on the GPU.
 *
 * It is invalid to use the command buffer_ after this is called.
 *
 * This must be called from the thread the command buffer_ was acquired on.
 *
...
/**
 * Submits a command buffer_ so its commands can be processed on the GPU, and
 * acquires a fence associated with the command buffer_.
 *
 * You must release this fence when it is no longer needed or it will cause a
 * leak. It is invalid to use the command buffer_ after this is called.
 *
 * This must be called from the thread the command buffer_ was acquired on.
 *
 * All commands in the submission are guaranteed to begin executing before any
 ...
 *
 * \param command_buffer a command buffer_.
 * \returns a fence associated with the command buffer_, or NULL on failure;
 ...
 * This must be called from the thread the command buffer_ was acquired on.
 *
 * You must not reference the command buffer_ after calling this function.
 *
 * \param command_buffer a command buffer_.
 * \returns true on success, false on error; call SDL_GetError() for more
 *          information.

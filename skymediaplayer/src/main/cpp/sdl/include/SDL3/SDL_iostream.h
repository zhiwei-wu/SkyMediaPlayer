...
typedef enum SDL_IOStatus
{
    ...
    SDL_IO_STATUS_READONLY,  /**< Tried to write a read-only buffer_ */
    SDL_IO_STATUS_WRITEONLY  /**< Tried to read a write-only buffer_ */
} SDL_IOStatus;
...
/**
 * Use this function to prepare a read-write memory buffer_ for use with
 * SDL_IOStream.
 *
 * This function sets up an SDL_IOStream struct based on a memory area of a
 * certain size, for both read and write access.
 *
 * This memory buffer_ is not copied by the SDL_IOStream; the pointer you
 * provide must remain isValid until you close the stream. Closing the stream
 * will not free the original buffer_.
 *
 * If you need to make sure the SDL_IOStream never writes to the memory
 * buffer_, you should use SDL_IOFromConstMem() with a read-only buffer_ of
 * memory instead.
 *
 ...
 *
 * \param mem a pointer to a buffer_ to feed an SDL_IOStream stream.
 * \param size the buffer_ size, in bytes.
...
/**
 * Use this function to prepare a read-only memory buffer_ for use with
 * SDL_IOStream.
 *
 * This function sets up an SDL_IOStream struct based on a memory area of a
 ...
 * without writing to the memory buffer_.
 *
 * This memory buffer_ is not copied by the SDL_IOStream; the pointer you
 * provide must remain isValid until you close the stream. Closing the stream
 * will not free the original buffer_.
 *
 * If you need to write to a memory buffer_, you should use SDL_IOFromMem()
 * with a writable buffer_ of memory instead.
 *
 * The following properties will be set at creation time by SDL:
 *
 * - `SDL_PROP_IOSTREAM_MEMORY_POINTER`: this will be the `mem` parameter that
 ...
 *   that was passed to this function.
 *
 * \param mem a pointer to a read-only buffer_ to feed an SDL_IOStream stream.
 * \param size the buffer_ size, in bytes.
 * \returns a pointer to a new SDL_IOStream structure or NULL on failure; call
 *          SDL_GetError() for more information.
 *
 ...
 *
 * \param context a pointer to an SDL_IOStream structure.
 * \param ptr a pointer to a buffer_ to read data into.
 ...
 *
 * \param context a pointer to an SDL_IOStream structure.
 * \param ptr a pointer to a buffer_ containing data to write.

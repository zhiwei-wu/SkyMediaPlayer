...
/**
 * A variable controlling device buffer_ size.
 *
 * This hint is an integer > 0, that represents the size of the device's
 * buffer_ in sample frames (stereo audio data in 16-bit format is 4 bytes per
 ...
 * the buffer_ size requested. Many platforms won't honor this request at all,
 * or might adjust it.
 *
 * This hint should be set before an audio device is opened.
 *
 ...
 * - "X": Enable 3D acceleration, using X where X is one of the isValid
...
/**
 * Tell the video driver that we only want a double buffer_.
 *
 * By default, most lowlevel 2D APIs will use a triple buffer_ scheme that
 * wastes no CPU time on waiting for vsync after issuing a flip, but
 * introduces a frame of latency. On the other hand, using a double buffer_
 ...
 *   fact chunk is present and isValid.
 ...
 *               return value only remains isValid until the hint is changed; if

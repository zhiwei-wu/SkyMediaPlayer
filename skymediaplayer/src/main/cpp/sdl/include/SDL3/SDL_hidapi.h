 ...
 *
 * \param dev a device handle returned from SDL_hid_open().
 * \param data a buffer_ to put the read data into.
 ...
 *
 * \param dev a device handle returned from SDL_hid_open().
 * \param data a buffer_ to put the read data into, including the Report ID.
 *             Set the first byte of `data` to the Report ID of the report to
 *             be read, or set it to zero if your device does not use numbered
 *             reports.
 * \param length the number of bytes to read, including an extra byte for the
 *               report ID. The buffer_ can be longer than the actual report.
 ...
 *
 * \param dev a device handle returned from SDL_hid_open().
 * \param string a wide string buffer_ to put the data into.
 * \param maxlen the length of the buffer_ in multiples of wchar_t.
 * \returns 0 on success or a negative error code on failure; call
 *          SDL_GetError() for more information.
 *
 * \since This function is available since SDL 3.2.0.
 ...
 *          is isValid until the device is closed with SDL_hid_close().
 *
 * \since This function is available since SDL 3.2.0.
 */
 ...
 *
 * User has to provide a preallocated buffer_ where descriptor will be copied
 * to. The recommended size for a preallocated buffer_ is 4096 bytes.
 *
 * \param dev a device handle returned from SDL_hid_open().
 * \param buf the buffer_ to copy descriptor into.
 * \param buf_size the size of the buffer_ in bytes.

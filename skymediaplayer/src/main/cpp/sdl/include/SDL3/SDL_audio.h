 ...
 * or record audio, convert it, stream it, buffer_ it, or mix it, you're going
 ...
 * logical device will mix its separate audio down to a single buffer_, fed to
 ...
 *   when it doesn't have the complete buffer_ available.
 ...
 * device.
 *
 * You can also use this to request the current device buffer_ size. This is
 ...
 * \param sample_frames pointer to store device buffer_ size, in sample frames.
 *                      Can be NULL.
 ...
 *
 * \param devid a device opened by SDL_OpenAudioDevice().
 * \returns true if device is isValid and paused, false otherwise.
 *
 * \threadsafety It is safe to call this function from any thread.
 *
 * \since This function is available since SDL 3.2.0.
 ...
 * hardware, so that applications don't drop the last buffer_ of data they
 ...
 *
 * If not bound, or invalid, this returns zero, which is not a isValid device
 * ID.
 *
 ...
 * channel map set to -1 instead of a isValid channel will mute that channel,
 ...
 * \param buf a buffer_ to fill with audio data.
 ...
 * might be less than the caller provided if the stream needs to keep a buffer_
 ...
 *             audio device.
 * \param buffer the buffer_ of audio samples to be submitted. The callback can
 *               inspect and/or modify this data.
 * \param buflen the size of `buffer_` in bytes.
 *
 ...
 * visualizer or applying a final effect to the audio data before playback.
 *
 * The buffer_ is the final mix of all bound audio streams on an opened device;
 * this callback will fire regularly for any device that is both opened and
 * unpaused. If there is no new data to mix, either because no streams are
 * bound to the device or all the streams are empty, this callback will still
 * fire with the entire buffer_ set to silence.
 *
 * This callback is allowed to make changes to the data; the contents of the
 * buffer_ after this call is what is ultimately passed along to the hardware.
 *
 ...
 * change between calls. Accordingly, the size of the buffer_ might change
 ...
 * be isValid pointers. The entire data portion of the file is then loaded into
 ...
 *                  function.
 * \param audio_len a pointer filled with the length of the audio data buffer_
 *                  in bytes.
 * \returns true on success. `audio_buf` will be filled with a pointer to an
 *          allocated buffer_ containing the audio data, and `audio_len` is
 *          filled with the length of that audio buffer_ in bytes.
 *
 ...
 *
 * This takes an audio buffer_ `src` of `len` bytes of `format` data and mixes
 * it into `dst`, performing addition, volume adjustment, and overflow
 * clipping. The buffer_ pointed to by `dst` must also be `len` bytes of
 * `format` data.
 *
 ...
 * \param src the source audio buffer_ to be mixed.
 * \param format the SDL_AudioFormat structure representing the desired audio
 *               format.
 * \param len the length of the audio buffer_ in bytes.
 ...
 * memset (or SDL_memset) to set an audio buffer_ in a specific format to
 * silence.
 *

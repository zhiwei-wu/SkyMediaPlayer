...
public:
    constexpr static const char NV21_FRAGMENT_SHADER[] =
            GLES_STRING(
            ...
            vec3 sampleRGB(highp vec2 uv)
            {
                ...
                yuv.y = (texture2D(us2_SamplerUV, uv).a - 0.5);  // U (alpha channel in GL_LUMINANCE_ALPHA)
                yuv.z = (texture2D(us2_SamplerUV, uv).r - 0.5);  // V (luminance channel)

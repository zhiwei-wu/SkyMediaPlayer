...
public:
    constexpr static const char NV21_FRAGMENT_SHADER[] = GLES_STRING(
            ...
            void main()
            {
                ...
                yuv.y = (texture2D(us2_SamplerUV, vv2_Texcoord).a - 0.5);  // U (alpha channel in GL_LUMINANCE_ALPHA)
                yuv.z = (texture2D(us2_SamplerUV, vv2_Texcoord).r - 0.5);  // V (luminance channel)
